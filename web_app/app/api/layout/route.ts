...
export async function PUT(request: Request) {
  try {
    ...
    if (!Array.isArray(positions)) {
      return NextResponse.json({ error: "Expected an array of positions" }, { status: 400 })
    }
    ...
    for (const pos of positions) {
      if (
        ...
        typeof pos.w !== "number" || !Number.isFinite(pos.w) || pos.w < 1 ||
        typeof pos.h !== "number" || !Number.isFinite(pos.h) || pos.h < 1
      ) {
