...
export async function POST(request: Request) {
  try {
    ...
    if (!body.cardId || !body.name || !body.date) {
      return NextResponse.json({ error: "cardId, name, and date are required" }, { status: 400 })
    }
    if (typeof body.calories !== "number" || !Number.isFinite(body.calories) || body.calories < 0) {
