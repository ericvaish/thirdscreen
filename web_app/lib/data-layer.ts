...
export async function getSetting(key: string): Promise<unknown> {
  const all = (await getSettings()) as Record<string, unknown> | undefined
  return all?.[key] ?? null
}
...
export async function listDashboards(): Promise<DashboardConfig[]> {
  ...
  try {
    ...
    console.error("[data-layer] listDashboards API failed, falling back to local:", e)
  }
...
export async function createDashboard(name: string): Promise<DashboardConfig> {
  ...
  try {
    ...
    console.error("[data-layer] createDashboard API failed, falling back to local:", e)
...
export async function updateDashboard(
  ...
  if (!isLocal) {
    try {
      ...
      console.error("[data-layer] updateDashboard API failed:", e)
    }
  }
...
export async function deleteDashboard(id: string): Promise<void> {
  ...
  if (!isLocal) {
    try {
      ...
      console.error("[data-layer] deleteDashboard API failed:", e)
    }
  }
}
...
export async function listNotifications(opts?: { unreadOnly?: boolean; limit?: number }) {
  if (isLocal) return local.localListNotifications(opts)
  if (isElectron) return ipc("db:notifications:list", opts)
...
export async function createNotification(data: {
  ...
  if (isLocal) return local.localCreateNotification(data)
  if (isElectron) return ipc("db:notifications:create", data)
...
export async function updateNotification(data: { id: string; read?: boolean }) {
  if (isLocal) return local.localUpdateNotification(data)
  if (isElectron) return ipc("db:notifications:update", data)
  return api("/api/notifications", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
...
export async function markAllNotificationsRead() {
  if (isLocal) return local.localMarkAllRead()
  if (isElectron) return ipc("db:notifications:markAllRead")
  return api("/api/notifications", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
export async function listCustomCharacters() {
  // Custom characters are always local for now
  return local.localListCustomCharacters()
}

export async function saveCustomCharacter(data: {
  id?: string
  name: string
  emoji: string
  ...
  return local.localSaveCustomCharacter(data as Parameters<typeof local.localSaveCustomCharacter>[0])
}
