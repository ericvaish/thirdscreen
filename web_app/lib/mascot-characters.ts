import type { MascotState, BuiltinMascotCharacter } from "./mascot"

function row(s: string): string[] { return s.split("") }
function frame(rows: string[]): string[][] { return rows.map(row) }

export const PALETTES: Record<BuiltinMascotCharacter, Record<string, string | null>> & Record<string, Record<string, string | null>> = {
  robot: {
...
export const CHARACTER_FRAMES: Record<BuiltinMascotCharacter, Record<MascotState, string[][][]>> & Record<string, Record<MascotState, string[][][]>> = {
  robot: {
