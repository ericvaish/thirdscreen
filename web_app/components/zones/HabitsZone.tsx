...
export function HabitsZone() {
  ...
  const { data: habitsData, setData: setHabitsData, refetch: refetchHabits } = useDataFetch(
    () => listHabits(format(subDays(new Date(), DAYS_TO_SHOW - 1), "yyyy-MM-dd"), today),
    [today]
  )
  ...
  const handleDelete = async (id: string) => {
    setHabitsData((prev) => prev ? { ...prev, habits: prev.habits.filter((h) => h.id !== id) } : prev)
    try {
  ...
  const handleToggle = async (habitId: string, date: string) => {
    setHabitsData((prev) => {
      ...
      const logs = existing
        ? prev.logs.filter((l) => l !== existing)
        : [...prev.logs, { id: `temp-${Date.now()}`, habitId, date, completed: true }]
      return { ...prev, logs }
    })
    try {
