...
import { AlignLeft, Shapes } from "lucide-react"
        ...
        <ScrollArea className="min-h-0 flex-1">
          <div className="space-y-6 px-4 pb-8">
            ...
            <section>
              ...
              <div className="mt-2 grid grid-cols-2 gap-1.5">
                {([
                  { value: "line" as ZoneLabelStyle, label: "Line", desc: "Colored bar", Icon: AlignLeft },
                  { value: "icon" as ZoneLabelStyle, label: "Icon", desc: "Zone icon", Icon: Shapes },
                  ...
                  <button
                    ...
                    onClick={() =>
                      updateTheme((t) => ({
                        ...t,
                        zoneLabelStyle: value,
                        preset: null,
                      }))
                    }
                    className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-left transition-all active:scale-[0.98] ${
                      (theme.zoneLabelStyle ?? "line") === value
                        ? "border-primary/40 bg-primary/10"
                        : "border-border/20 bg-muted/10 hover:border-border/30 hover:bg-muted/15"
                    }`}
                  >
                    <Icon className={`size-4 shrink-0 ${
                      (theme.zoneLabelStyle ?? "line") === value ? "text-primary" : "text-muted-foreground/50"
                    }`} />
                    <div>
                      <span className={`text-xs font-semibold ${
                        (theme.zoneLabelStyle ?? "line") === value ? "text-primary" : "text-foreground/80"
                      }`}>
                        {label}
                      </span>
