...
export function TimelineZone() {
  ...
  // Derived window bounds: current time is always centered (day view only)
  ...
  // Current time is always at 50% (day view)
  const nowPercent = 50
          ...
          <div className="flex items-center gap-0.5">
            ...
            <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
              <PopoverTrigger asChild>
                <button className="flex items-center gap-1 rounded px-1.5 py-0.5 text-[0.625rem] font-medium text-muted-foreground/70 transition-colors hover:bg-muted/30 hover:text-foreground/80">
            ...
            {showTodayButton && (
              <button
                onClick={goToToday}
                className="rounded border border-border/20 px-1.5 py-0.5 font-mono text-[0.5rem] font-semibold uppercase tracking-wider text-muted-foreground/50 transition-colors hover:border-amber-400/30 hover:text-amber-400"
              >
                Today
              </button>
            )}
          ...
          <div className="flex items-center rounded-md border border-border/20 bg-muted/20 p-0.5">
            {(["day", "week", "month"] as ViewMode[]).map((mode) => (
              <button
                ...
                className={`rounded-sm px-2 py-0.5 font-mono text-[0.5625rem] font-semibold uppercase tracking-wider transition-all ${
                  viewMode === mode
                    ? "bg-[var(--zone-timeline-accent)]/15 text-[var(--zone-timeline-accent)] shadow-sm"
